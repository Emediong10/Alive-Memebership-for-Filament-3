<x-filament-panels::page>
    ...
    $licence = App\Models\User::where(['status'=>0,'id'=>auth()->user()->id])
                            ...
                            //  ->whereNull('professional_abilities')
                            //   ->orWhereNull('mission_id')
                            //   ->whereNull('spiritual_gift_id')
                            //  ->whereNull('skill_id')
                            //   ->whereNull('area_interest_id')
                              ->first();
    $user = auth()->user();
    //dd($licence);
@endphp
...
{{-- {{ dd($licence) }} --}}
            ...
            <div class="center" style="font-size:22px; font-weight:bold">
                <p>If you are not sure of your spiritual gifts, click below to complete the Spiritual Gifts test. Take note of your first 3 Spiritual gifts.</p>
            </div>
             <div class="alert-info center" style="font-size:22px; font-weight:bold" >
            <span class="closebtn" onclick="this.parentElement.style.display='none';">&times;</span>
            <a href="http://spiritualgiftstest.com/spiritual-gifts/" target='blank'>Click here to take Your Spiritual Gift Test</a>
            </div>
...
.alert {
padding: 20px;
background-color: rgb(13, 53, 16); /* Red */
color: white;
margin-bottom: 15px;
}
.alert-info {
padding: 20px;
background-color: rgb(11, 28, 53); /* Red */
color: white;
margin-bottom: 15px;
}
